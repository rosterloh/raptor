...
| Dynamic rollouts (`dynamic`, `dynamicGroupTemplate`; trailing group absorbs newly-matching targets, rolls over at capacity, never self-completes) | ✅ (`amountGroups: 0` "pure dynamic" rollouts, with no static groups at all, are not accepted — [#147](https://github.com/rosterloh/raptor/issues/147)) |
