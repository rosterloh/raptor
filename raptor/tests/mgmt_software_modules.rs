...
async fn create_list_get_delete_module() {
    ...
    let resp = app
        ...
        .oneshot(common::req(
            ...
            Some(json!([{"name": "rootfs", "version": "1.0.0", "type": "os", "vendor": "acme"}])),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(common::req(
            "GET",
            "/rest/v1/softwaremodules?q=name==root*",
            None,
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(common::req(
            "GET",
            &format!("/rest/v1/softwaremodules/{id}"),
            None,
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(common::req(
            "PUT",
            &format!("/rest/v1/softwaremodules/{id}"),
            Some(json!({"description": "root filesystem"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
...
async fn duplicate_module_conflicts_and_unknown_type_rejected() {
    ...
    assert_eq!(
        app.clone()
            .oneshot(common::req("POST", "/rest/v1/softwaremodules", Some(m)))
            .await
            .unwrap()
            .status(),
        StatusCode::CONFLICT
    ...
    assert_eq!(
        app.clone()
            .oneshot(common::req(
                "POST",
                "/rest/v1/softwaremodules",
                Some(json!([{"name": "x", "version": "1", "type": "nope"}]))
            ))
            .await
            .unwrap()
            .status(),
...
async fn type_endpoints_are_seeded_and_readonly() {
    ...
    let resp = app
        .clone()
        .oneshot(common::req("GET", "/rest/v1/softwaremoduletypes", None))
        .await
        .unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let body = common::body_json(resp).await;
    ...
    let resp = app
        .clone()
        .oneshot(common::req("GET", "/rest/v1/distributionsettypes", None))
        .await
        .unwrap();
