...
pub async fn type_keys(db: &sea_orm::DatabaseConnection) -> Result<HashMap<i64, String>, AppError> {
    Ok(software_module_type::Entity::find()
        .all(db)
        .await?
        .into_iter()
        .map(|t| (t.id, t.key))
...
pub async fn create(
    ...
    for c in body {
        let ty = software_module_type::Entity::find()
            ...
            .await?
            .ok_or_else(|| {
                AppError::BadRequest(format!("unknown software module type: {}", c.module_type))
            })?;
        ...
        if dup.is_some() {
            return Err(AppError::Conflict(format!(
                "software module {}:{} already exists",
                c.name, c.version
            )));
        }
        let now = now_ms();
...
pub async fn get_one(
    ...
    let m = software_module::Entity::find_by_id(id)
        .one(&st.db)
        .await?
