...
pub async fn sm_types(
    ...
    let (rows, total) =
        crate::api::paging::page(&st.db, software_module_type::Entity::find(), &p).await?;
    Ok(Json(Paged::new(
        rows.iter()
            .map(|t| type_json(t.id, &t.key, &t.name, "softwaremoduletypes"))
            .collect(),
        total,
    )))
}
...
pub async fn ds_types(
    ...
    let (rows, total) =
        crate::api::paging::page(&st.db, distribution_set_type::Entity::find(), &p).await?;
    Ok(Json(Paged::new(
        rows.iter()
            .map(|t| type_json(t.id, &t.key, &t.name, "distributionsettypes"))
            .collect(),
        total,
    )))
}
